      ...
      // children: [
        ClipRect(
          child: RepaintBoundary(
            child: Tooltip(
              message: widget.nameJP,
              textStyle: const TextStyle(color:Colors.white,fontSize: 12,fontFamily: ConfigCustom.font_family),
              child: Container(
                ...
                child: Stack(
                  children: [
                    currentValueNotifier.value == 0.0 ? Container() :
                    Positioned(
        ...
        // Text('${widget.hiveValue.toStringAsFixed(2)} | ${widget.startValue.toStringAsFixed(2)}->${widget.endValue.toStringAsFixed(2)}',style:TextStyle(color:Colors.white))
        // ],
    // );
  }
...
//   final result = durationMs.round().clamp(10, ConfigCustom.maxTimeToStartAnDecimalAnimationMs);
...
int calculationDurationPerStep({
  ...
  final totalSteps = ((endValue - startValue) / 0.01).ceil();
  // Preliminary duration to determine totalDuration
  final preliminaryDurationMs = (30.0 * 1000) / totalSteps;
  final double totalDuration = preliminaryDurationMs < 1000 ? 30.0 : 30.0;

  // Calculate duration per step, ensuring estimatedTotal <= 30.0
  final double maxDurationMs = (totalDuration * 1000) / totalSteps;
  // Cap durationMs to ensure total duration doesn't exceed 30 seconds
  final double cappedDurationMs = (30.0 * 1000) / totalSteps;
  final double durationMs = maxDurationMs < cappedDurationMs ? maxDurationMs : cappedDurationMs;
  final int result = durationMs.round().clamp(10, ConfigCustom.maxTimeToStartAnDecimalAnimationMs);

  // Verify estimated total duration
  final double estimatedTotal = totalSteps * result / 1000;
  // debugPrint('Odometer: Calculation - startValue: $startValue, endValue: $endValue, totalSteps: $totalSteps, durationMs: $durationMs, rounded: $result, totalDuration: $totalDuration, estimatedTotal: ${estimatedTotal}s');

  // If estimatedTotal exceeds 30s, adjust result downward
  if (estimatedTotal > 30.0) {
    final adjustedResult = ((30.0 * 1000) / totalSteps).floor().clamp(10, ConfigCustom.maxTimeToStartAnDecimalAnimationMs);
    // debugPrint('Odometer: Adjusted - original result: $result, adjustedResult: $adjustedResult, new estimatedTotal: ${(totalSteps * adjustedResult / 1000)}s');
